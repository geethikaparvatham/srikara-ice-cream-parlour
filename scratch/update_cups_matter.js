...
const cupData = {
  "Classic Cups": ["Vanilla Delight Cup", "Chocolate Heaven Cup", "Strawberry Bliss Cup", "Butterscotch Crunch Cup", "Mango Magic Cup", "Kesar Pista Cup", "Black Currant Cup", "Coffee Mocha Cup", "Tender Coconut Cup", "Pineapple Paradise Cup"],
  "Premium Cups": ["Belgian Chocolate Cup", "Ferrero Fantasy Cup", "Dark Chocolate Truffle Cup", "Nutty Caramel Cup", "Almond Crunch Cup", "Roasted Hazelnut Cup", "Chocolate Brownie Cup", "Chocolate Fudge Cup", "Oreo Blast Cup", "KitKat Crunch Cup"],
  "Fruit Cups": ["Mixed Berry Cup", "Blueberry Burst Cup", "Lychee Love Cup", "Kiwi Fresh Cup", "Watermelon Splash Cup", "Peach Paradise Cup", "Dragon Fruit Cup", "Apple Cinnamon Cup", "Fruit Fiesta Cup", "Tropical Delight Cup"],
  "Kids Special Cups": ["Rainbow Sprinkle Cup", "Cotton Candy Cup", "Bubblegum Blast Cup", "Unicorn Magic Cup", "Choco Chip Party Cup", "Cookie Monster Cup", "Jelly Bean Cup", "Candy Crush Cup", "Mini Marshmallow Cup", "Funfetti Cup"],
  "Dessert Cups": ["Red Velvet Cup", "Tiramisu Cup", "Cheesecake Cup", "Brownie Blast Cup", "Black Forest Cup", "Choco Lava Cup", "Caramel Pudding Cup", "Walnut Brownie Cup", "Cookie Dough Cup", "Death by Chocolate Cup"],
  "Dry Fruit & Nut Cups": ["Royal Kaju Cup", "Pista Royale Cup", "Badam Delight Cup", "Dry Fruit Supreme Cup", "Honey Almond Cup", "Cashew Crunch Cup", "Nutty Treasure Cup", "Anjeer Delight Cup", "Dates & Nuts Cup", "Royal Maharaja Cup"]
};
...
const getDynamicImage = (flavorName, index) => {
  ...
  else if (name.includes('candy') || name.includes('rainbow') || name.includes('unicorn') || name.includes('bubblegum') || name.includes('jelly') || name.includes('funfetti')) pool = imagePool.kids;
...
const getUniqueDescription = (name) => {
  ...
  if (n.includes('vanilla')) return "A timeless classic featuring rich, creamy vanilla bean perfection in a premium cup.";
  if (n.includes('dark chocolate')) return "Dive into deep, indulgent dark chocolate crafted from premium bitter cocoa.";
  if (n.includes('belgian')) return "Luxurious Belgian chocolate swirls folded into a refreshing cool cup.";
  ...
  if (n.includes('kiwi') || n.includes('lychee') || n.includes('peach') || n.includes('dragon')) return "A refreshing, exotic fruit sensation that brings the taste of summer to your cup.";
  ...
  const finishes = ["served in our signature eco-friendly cup.", "topped with a touch of magic.", "crafted for ultimate indulgence.", "guaranteed to satisfy your sweet tooth.", "perfectly balanced for a rich taste."];
  
  const adj = adjectives[name.length % adjectives.length];
  const fin = finishes[name.length % finishes.length];
  
  return adj + ' ' + name.replace('Cup', '').trim().toLowerCase() + ' ice cream, ' + fin;
};
...
const badges = {
  "Classic Cups": "Classic",
  "Premium Cups": "Premium",
  "Fruit Cups": "Fruity",
  "Kids Special Cups": "Kids Fav",
  "Dessert Cups": "Dessert",
  "Dry Fruit & Nut Cups": "Nutty"
};

const colors = {
  "Classic Cups": "from-[#F5EAD4] to-[#DFD0B8]",
  "Premium Cups": "from-[#4E2F1D] to-[#2B1B10]",
  "Fruit Cups": "from-[#FF8DA1] to-[#FF4E72]",
  "Kids Special Cups": "from-[#005BFF] to-[#0B2E59]",
  "Dessert Cups": "from-[#FFB200] to-[#FF7A00]",
  "Dry Fruit & Nut Cups": "from-[#7FA13B] to-[#4F6C1D]"
};

let outputData = Object.keys(cupData).map(cat => {
  return {
    title: cat,
    icon: icons[cat],
    flavours: cupData[cat].map((flavour, idx) => ({
      name: flavour,
...
const CUP_CATEGORIES = ${JSON.stringify(outputData, null, 2).replace(/"([^"]+)":/g, '$1:')};
...
fs.writeFileSync(path.join('c:', 'Users', 'PARVATHAM GEETHIKA', 'OneDrive', 'Desktop', 'Ice Cream Parlour', 'src', 'components', 'flavours', 'Cups.tsx'), componentCode);
console.log("Cups.tsx updated with UNIQUE matter and variety of images!");
